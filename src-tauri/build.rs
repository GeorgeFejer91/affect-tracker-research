...
fn main() {
    ...
    println!("cargo:rerun-if-changed=tauri.runner.conf.json");
...
fn git_is_dirty(manifest_dir: &std::path::Path) -> Result<bool, String> {
    git_output(
        manifest_dir,
...
fn git_output(manifest_dir: &std::path::Path, arguments: &[&str]) -> Result<String, String> {
