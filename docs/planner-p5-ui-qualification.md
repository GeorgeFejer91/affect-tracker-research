...
Tested edits themselves use the rendered controls. Their oracle is the real typed
P5 adapter with detached editor hooks; this is explicitly not evidence for Main's
new production command-surface wiring or native stdin broker. The combined-source
rerun must add that real surface once Main freezes it. Main also owns the complete
JSON/file mock and native media checks. Runner correspondence is active under
