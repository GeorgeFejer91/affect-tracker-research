...
| Q04 | Answered / P1 | Use the exact relative location including filename extension. v2 escapes `%`, `_` and spreadsheet-dangerous leading characters reversibly, requires already-NFC safe components, and treats collisions as errors. Rename/move changes the location ID; equal bytes keep the same `assetId`. | References carry `{assetId, annotationId}` so equal content at two paths remains two catalogue locations. Historical v1 readers retain their prior meaning. |
