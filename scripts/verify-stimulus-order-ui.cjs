...
(async () => {
  ...
  try {
    ...
    const receipt = await page.evaluate(async () => {
      ...
      const fixture = await (await fetch("/test/fixtures/stimulus-order-v1.json")).json();
      ...
      controller.destroy();
      root.innerHTML = renderResearchUiMarkup();
      const editor = createStimulusOrderEditor({ root, operate: async () => ({ library: fixture.library, design: fixture.design }) });
      await editor.adopt({ library: fixture.library, design: fixture.design }, { loadSaved: true });
    ...
    const fixture = JSON.parse(await readFile(path.join(root, "test/fixtures/stimulus-order-v1.json")));
    assert.equal(receipt.initialized, true);
    assert.equal(receipt.columns, 2); assert.equal(receipt.rows, 5);
    assert.deepEqual(receipt.values, fixture.design.rows.flat());
    ...
    assert.deepEqual(receipt.snapshot, fixture.design);
    ...
    console.log(JSON.stringify({ ...receipt, snapshot: undefined, contained, errors, output }));
