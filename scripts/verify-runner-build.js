...
const expectedKinds = ["index.html", "runner.js", "vdoninja-sdk.min.js", "runner.css", "runner-symbol.svg", "app-symbol.svg", "controller-qr.svg", "professor-widget.svg", "input-widget.svg", "remote-widget.svg"];
const sharedCss = await readFile(resolve(dist, files.find(file => file.endsWith(".css"))), "utf8");
for (const theme of ["dark", "light"]) {
  if (sharedCss.includes(`flubber-input-${theme}-`)) expectedKinds.push(`flubber-input-${theme}.svg`);
}
const actualKinds = files.map(file => file === "index.html" ? file : file.replace(/^assets\/(vdoninja-sdk\.min|professor-widget|input-widget|remote-widget|professor-qr|controller-qr|runner-symbol|app-symbol|flubber-input-dark|flubber-input-light|runner)-[\w-]+\.(js|css|svg)$/u, "$1.$2"));
