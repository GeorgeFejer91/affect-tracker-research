...
assert.ok(["reset","restore","labels-axes","labels-corners","stage-default","stage-restored"].includes(mode),"Unknown preview fixture mode");
...
const fixtureArgument = mode.startsWith("labels-") ? JSON.stringify(mode.slice(7))
  : mode.startsWith("stage-") ? JSON.stringify(mode.slice(6)) : "";
