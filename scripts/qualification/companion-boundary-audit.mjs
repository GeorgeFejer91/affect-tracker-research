...
try {
 let root;
 if(program==='planner') {
  document.body.innerHTML='<div id="research-app" data-research-surface="tauri" data-research-program="planner"></div>';
  root=bootResearchUi({surface:'tauri'}); root.researchUi.openSetupSection('review');
  ...
  await new NativeResearchRuntimeBridge(root,{invoke}).initialize();
 } else {
...
const server=createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/'){res.setHeader('Content-Type','text/html');res.end('<!doctype html><meta charset="utf-8"><link rel="stylesheet" href="'+(url.searchParams.get('program')==='planner'?'/site/research.css':'/runner/runner.css')+'"><script type="module" src="/runner/src/audit.js"></script>');return;}
...
const rows=[];
try {for(const program of ['planner','runner']){
 ...
 if(program==='planner'){assert.equal(row.runButtons,0);assert.equal(row.reviewTitle,'Review & Export');assert.ok(row.previewCount>=1);assert.ok(row.sections>=7);}
