...
const [browser, destination, viewport="1938,1176", suite="flubber"] = process.argv.slice(2);
...
const invoke=async(command,args)=>{calls.push(command);switch(command){
 ...
 case 'research_runner_master_action':{
  ...
  if(action.type==='presented'){
   ...
   if((mode==='isi-neutral'||representative)&&plan.steps[status.position-1].kind==='interval'){
  ...
  else if(action.type==='submit'){status.position=(mode==='isi-neutral'||representative)?2:4;status.phase='awaitingPresentation';status.answers={};}
...
try{
 ...
 let bytes=new Uint8Array(await(await fetch('/test/fixtures/planner-recipe-locations-current-v1.canonical.json')).arrayBuffer());
 if(representative){
  const {integrity,...core}=JSON.parse(new TextDecoder().decode(bytes));
  core.recipeId='synthetic-visible-flubber-projection';
  core.segments.P4.reference.box={width:60,height:60};core.segments.P4.reference.centre={x:50,y:35};
  core.segments.P4.feedback={minimumGap:3,offset:{x:0,y:75},origin:'design-centre',overlayViewportSide:24};
  core.segments.P5.presentation.renderer='flubber';core.segments.P5.visual.hideFeedback=false;
  bytes=new TextEncoder().encode(await serializePlannerRecipeV1(await compilePlannerRecipeV1(core)));
 }await app.adoptRecipe(bytes);
 ...
 if(mode==='sequence'){
  ...
  if(mode==='video'){
  ...
  }else if((mode==='isi-neutral'||representative)){
   ...
   const capture=async(label)=>{
    await tick();
    const stage=root.querySelector('.run-feedback-stage'),svg=root.querySelector('[data-preview-flubber]'),path=root.querySelector('[data-preview-flubber-base]'),r=stage.getBoundingClientRect(),sr=svg.getBoundingClientRect();
    const inner=root.querySelector('.research-preview-stage').getBoundingClientRect();
    const row={label,inner:{width:inner.width,height:inner.height},border:getComputedStyle(stage).borderWidth,expected:plan.selected.layout.geometry.feedback,stage:{x:r.x,y:r.y,width:r.width,height:r.height},svg:{x:sr.x,y:sr.y,width:sr.width,height:sr.height,display:getComputedStyle(svg).display,viewBox:svg.getAttribute('viewBox')},pathLength:path.getAttribute('d').length,path:path.getAttribute('d'),color:root.querySelector('[data-preview-overlay]').style.getPropertyValue('--flubber-color'),cached:structuredClone(globalThis.__runnerPreviewAuditState),hidden:stage.hidden};
    const expectedState=runnerMasterFeedbackState(plan.selected.feedback,label==='strong'?0.9:0,label==='strong'?-0.8:0);
    for(const key of ['x','y','frequency','edgeSmoothness','amplitude','pulseSynchrony','waveVariation','saturation'])check(row.cached[key]===expectedState[key],label+' mapped '+key);
    if(label==='strong')check(row.path!==renderGeometry[0].path&&row.color!==renderGeometry[0].color,'strong state changes visible path and color');
    (renderGeometry??=[]).push(row);
    if(!stage.hidden){check(row.border==='0px','no participant container chrome '+label);check(Math.abs(r.width-inner.width)<0.02&&Math.abs(r.height-inner.height)<0.02,'full authored box reaches renderer '+label);check(Math.abs(r.width-sr.width)<0.02&&Math.abs(r.height-sr.height)<0.02,'SVG uses full authored box '+label);for(const key of ['x','y','width','height'])check(Math.abs(row.expected[key]-r[key])<0.1,label+' P4 '+key);check(sr.width>50&&sr.height>50&&row.pathLength>100,'recognizable sized SVG '+label);}
   };
   if(representative){status={...status,position:video.position,phase:'awaitingPresentation',currentValence:0,currentArousal:0};await tick();await tick();await capture('neutral');}
   if(mode!=='flubber-neutral'){
   ...
   if(representative)await capture('strong');
   if(mode!=='flubber-strong'){
   ...
   if(representative)await capture('hidden-isi');
   if(mode!=='flubber-isi'){
   ...
   if(representative)await capture('next-video-neutral');
   }}}
  }else if(mode==='stop'){
...
const result=document.createElement('pre');result.id='receipt';result.hidden=true;result.textContent=JSON.stringify({mode,checks,errors,renderGeometry,viewport:[innerWidth,innerHeight],calls,scope:representative?'Synthetic inputs and native replies; production renderer with test-only 16ms timer frame scheduler (headless RAF stalls); no native video, XDF, physical paint or timing qualification':'Synthetic frontend projections only; no real run or native qualification'});document.body.append(result);
`;
...
const bundle=await build({stdin:{contents:entry,resolveDir:root,sourcefile:'master-ui-audit.js'},bundle:true,format:'esm',write:false,platform:'browser',logLevel:'silent',plugins:[{name:'preview-state-observer',setup(builder){builder.onLoad({filter:/[\\/]research[\\/]preview\.js$/},async({path})=>{const source=await readFile(path,'utf8'),needle='state = normalizedState({ ...state, ...nextState, ...square });';assert.ok(source.includes(needle));return{contents:source.replace(needle,needle+' globalThis.__runnerPreviewAuditState = structuredClone(state);').replaceAll('requestAnimationFrame(renderFrame)', suite==='flubber'?'setTimeout(() => renderFrame(performance.now()), 16)':'requestAnimationFrame(renderFrame)').replaceAll('cancelAnimationFrame(frameId)',suite==='flubber'?'clearTimeout(frameId)':'cancelAnimationFrame(frameId)'),loader:'js',resolveDir:resolve(path,'..')};});}}]});
...
try{for(const mode of (suite==='flubber'?['flubber-neutral','flubber-strong','flubber-isi','flubber-next']:['launcher','sequence','questionnaire','video','isi-neutral','stop'])){
