...
const bundle=await build({entryPoints:['test/fixtures/questionnaire-visual-browser.js'],bundle:true,write:false,metafile:true,format:'iife',target:'chrome105',loader:{'.csv':'text'},logLevel:'silent',define:{'import.meta.url':JSON.stringify(pathToFileURL(resolve('experiment-planner/web/src/research/app.js')).href)}});
const css=await readFile('experiment-planner/web/research.css','utf8');
const sourceFiles={};
for (const path of [...Object.keys(bundle.metafile.inputs),'experiment-planner/web/research.css','scripts/qualification/questionnaire-visual.mjs'].sort()) sourceFiles[path]=createHash('sha256').update(await readFile(path)).digest('hex');
