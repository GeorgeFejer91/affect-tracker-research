...
const [browser, destination, viewport="1938,1176", suite="grid"] = process.argv.slice(2);
 ...
 if(mode==='sequence'){
  ...
  if(mode==='video'){
   ...
   const capture=async(label)=>{
    ...
    const row={label,expected:plan.selected.layout.geometry.feedback,stage:{x:r.x,y:r.y,width:r.width,height:r.height},svg:{x:sr.x,y:sr.y,width:sr.width,height:sr.height,display:getComputedStyle(svg).display,viewBox:svg.getAttribute('viewBox')},pathLength:path.getAttribute('d').length,path:path.getAttribute('d'),color:root.querySelector('[data-preview-overlay]').style.getPropertyValue('--flubber-color'),cached:structuredClone(globalThis.__runnerPreviewAuditState),hidden:stage.hidden};
    ...
    if(!stage.hidden){for(const key of ['x','y','width','height'])check(Math.abs(row.expected[key]-r[key])<0.1,label+' P4 '+key);check(sr.width>50&&sr.height>50&&row.pathLength>100,'recognizable sized SVG '+label);}
   };
