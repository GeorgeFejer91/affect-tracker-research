...
// node scripts/qualification/surveyjs-rendered.mjs <browser.exe> <new-output-dir> [width] [populated|actions|invalid]
...
let receipt;
const html = `<!doctype html><meta charset="utf-8"><title>SurveyJS Planner and Runner</title><link rel="stylesheet" href="/site/research.css"><main></main><pre id="receipt" hidden></pre>
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    } else if (pathname === "/local-tas.csv" && process.env.AFFECT_LOCAL_TAS_SOURCE) {
      const bytes = await readFile(process.env.AFFECT_LOCAL_TAS_SOURCE);
      assert.equal(hash(bytes), "7b32c878cf83d2b0348498355402f1a8d1db5853aeffeaf2f74863ea701eef92");
      response.setHeader("Content-Type", "text/csv"); response.end(bytes);
...
try {
  const profile = await mkdtemp(join(output, "isolated-profile-")), screenshot = join(output, "surveyjs.png");
