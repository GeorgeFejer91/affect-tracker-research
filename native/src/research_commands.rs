...
mod catalogue_export_tests {
    ...
    use crate::research_workspace::{
        DecodeAttestationKind, DecodeAttestationRequest, DecodeBackend,
    };
    use std::cell::Cell;
...
/// Shared read-only intake for both companion programs. No workspace permission
/// or runtime is created by selecting a recipe.
#[tauri::command]
...
#[tauri::command]
pub async fn research_save_planner_recipe(
    window: WebviewWindow,
    app: AppHandle,
    role: State<'_, crate::research_desktop::DesktopRole>,
    source_text: String,
...
#[tauri::command]
pub async fn research_prepare_html_video_assets(
    window: WebviewWindow,
    workspace: State<'_, Arc<WorkspaceService>>,
    workspace_id: String,
    stimuli: Vec<HtmlVideoAssetRequest>,
) -> ResearchResult<HtmlVideoAssetPreparationReceipt> {
    authorize(&window)?;
    let workspace = Arc::clone(&workspace);
    tauri::async_runtime::spawn_blocking(move || {
        workspace.prepare_html_video_assets(&workspace_id, stimuli)
    })
    .await
    .map_err(CommandError::io)?
}
...
fn validate_protocol_run_envelope(
    ...
    if playback_mode == PlaybackMode::LegacyNativePlayer {
        return Err(CommandError::invalid_contract(
            "Only WebView HTML video playback can start a questionnaire protocol.",
        ));
    }
