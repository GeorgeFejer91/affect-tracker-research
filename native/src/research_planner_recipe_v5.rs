...
pub struct QuestionnaireAssetSnapshot {
...
mod tests {
    ...
    fn manifest5_matches_js_content_and_every_variant_language_plan() {
        ...
        let old = parse(include_bytes!(
            "../../test/fixtures/planner-recipe-v4-surveyjs.canonical.json"
        ))
        .unwrap();
        let old_source = old.canonical_source_text.clone();
        assert_eq!(
            loaded.wire_document().unwrap()["resolvedRecipe"],
            serde_json::to_value(&old.recipe).unwrap()
        );
        ...
        for case in expected.as_array().unwrap() {
            ...
            assert_eq!(
                ...
                old.recipe.reconstruct_selection(&selector).unwrap()
            );
            ...
            .unwrap();
            assert_eq!(
                canonical_json(&prepared.plan, &[]).unwrap(),
                canonical_json(case, &[]).unwrap()
            );
            let historical = crate::research_runner_master::PreparedMaster::read(
                &old_source,
                "P001",
                serde_json::from_value(selector).unwrap(),
            )
            .unwrap();
            assert_eq!(
                canonical_json(&prepared.plan.steps, &[]).unwrap(),
                canonical_json(&historical.plan.steps, &[]).unwrap()
            );
        }
    ...
    fn manifest5_rejects_changed_missing_reordered_extra_or_unsafe_assets() {
        ...
        let assets: Vec<QuestionnaireAssetSnapshot> =
            serde_json::from_value(bundle["questionnaireAssets"].clone()).unwrap();
        let mut changed = assets.clone();
        changed[0].source_text.push(' ');
        let mut reversed = assets.clone();
        reversed.reverse();
        let mut extra = assets.clone();
        extra.push(assets[0].clone());
        ...
        for path in [
            ...
            let mut reference = PlannerAssetManifestV5::read(MANIFEST.as_bytes())
                .unwrap()
                .references()
                .unwrap()
                .remove(0);
...
impl QuestionnaireAssetReference {
    pub fn validate(&self) -> ResearchResult<()> {
        ...
        if id.is_empty()
            ...
            || language.is_empty()
            || !(2..=8).contains(&language[0].len())
            || !language[0].bytes().all(|b| b.is_ascii_alphabetic())
            || language[1..].iter().any(|s| {
        ...
        let suffix = match self.format.as_str() {
            "surveyjs" => {
                ...
                if self.metadata["schema"] != "affect-research-surveyjs-definition"
                    || self.metadata["version"] != 1
                {
                    return Err(invalid("Unsupported SurveyJS asset metadata."));
                }
                "survey"
            }
            ...
            _ => {
                return Err(invalid(
                    "Unsupported questionnaire asset format or metadata.",
                ))
            }
        };
        let path = format!(
            "assets/questionnaires/{}/{}/{}.{}.json",
            self.questionnaire_id.to_ascii_lowercase(),
        ...
        if self.relative_path != path {
            return Err(invalid(
                "Questionnaire assets require an exact portable content-addressed path.",
            ));
        }
        Ok(())
    }
...
impl PlannerAssetManifestV5 {
    pub fn read(bytes: &[u8]) -> ResearchResult<Self> {
        ...
        let result: Self = serde_json::from_value(value.clone())
            .map_err(|_| invalid("Invalid questionnaire manifest fields."))?;
...
impl PlannerRecipeV5 {
    pub fn read(bytes: &[u8], assets: Vec<QuestionnaireAssetSnapshot>) -> ResearchResult<Self> {
        ...
        if refs.len() != assets.len() {
            return Err(invalid(
                "Load every declared questionnaire asset before opening the experiment.",
            ));
        }
    ...
    pub fn bundle_text(&self, source: &str) -> ResearchResult<String> {
        let mut bytes = canonical_json(
            &json!({"schema":BUNDLE_SCHEMA,"version":1,"recipeSourceText":source,"questionnaireAssets":self.assets}),
            &[],
        )?;
        bytes.push(b'\n');
