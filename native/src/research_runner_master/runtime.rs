...
#[serde(rename_all = "camelCase")]
pub(crate) struct MasterViewportProjection {
    pub(crate) mode: &'static str,
    pub(crate) viewport_matches: bool,
    pub(crate) scale: f64,
    pub(crate) reference: NativeMediaViewportCssV1,
}
...
fn css_number(value: &Value, key: &str) -> ResearchResult<f64> {
    value[key]
        .as_f64()
...
fn css_box(value: &Value) -> ResearchResult<CssBox> {
    Ok(CssBox {
        x: css_number(value, "x")?,
        y: css_number(value, "y")?,
        width: css_number(value, "width")?,
        height: css_number(value, "height")?,
        cy: css_number(value, "cy")?,
    })
}

fn centered_css_box(cx: f64, cy: f64, width: f64, height: f64) -> CssBox {
    CssBox {
        x: cx - width / 2.0,
        y: cy - height / 2.0,
        width,
...
fn css_viewport(box_: CssBox) -> NativeMediaViewportCssV1 {
    NativeMediaViewportCssV1 {
        left_css_px: box_.x,
        top_css_px: box_.y,
        width_css_px: box_.width,
        height_css_px: box_.height,
        layout_revision: 1,
    }
}

fn intended_vertical_gap(reference: CssBox, feedback: CssBox, minimum_gap: f64) -> f64 {
    if feedback.cy >= reference.cy {
        (feedback.y - (reference.y + reference.height)).max(minimum_gap)
    } else {
        (reference.y - (feedback.y + feedback.height)).max(minimum_gap)
    }
    .max(0.0)
}

pub(crate) fn native_viewport_projection(
    prepared: &PreparedMaster,
    (width, height, scale): (u32, u32, f64),
) -> ResearchResult<MasterViewportProjection> {
    let authored = &prepared.layout.viewport;
    if !scale.is_finite() || scale <= 0.0 {
        return Err(CommandError::invalid_contract(
            "The current presentation viewport is unavailable.",
        ));
    }
    let actual_width = f64::from(width) / scale;
    let actual_height = f64::from(height) / scale;
    if !actual_width.is_finite()
        || !actual_height.is_finite()
        || actual_width <= 0.0
        || actual_height <= 0.0
    {
        return Err(CommandError::invalid_contract(
    ...
    let reference = css_box(&geometry["reference"])?;
    let feedback = css_box(&geometry["feedback"])?;
    let viewport_matches =
        actual_width == authored.width_css_px && actual_height == authored.height_css_px;
    if viewport_matches {
        return Ok(MasterViewportProjection {
            mode: "authored",
            viewport_matches,
            scale: 1.0,
            reference: css_viewport(reference),
        });
    }

    let minimum_gap = geometry["gap"].as_f64().unwrap_or(0.0);
    let gap = intended_vertical_gap(reference, feedback, minimum_gap);
    let basis_width = reference.width.max(feedback.width);
    let basis_height = reference.height + gap + feedback.height;
    if basis_width <= 0.0 || basis_height <= 0.0 {
        return Err(CommandError::invalid_contract(
    ...
    let projection_scale = [
        1.0,
        actual_width / authored.width_css_px,
        actual_height / authored.height_css_px,
        actual_width / basis_width,
        actual_height / basis_height,
    ]
    .into_iter()
    .fold(f64::INFINITY, f64::min);
    if !projection_scale.is_finite() || projection_scale <= 0.0 {
        return Err(CommandError::invalid_contract(
            "The saved video and feedback layout cannot be projected into this viewport.",
        ));
    }
    let x = actual_width / 2.0;
    let top = ((actual_height - basis_height * projection_scale) / 2.0).max(0.0);
    let projected_reference = centered_css_box(
        x,
        top + reference.height * projection_scale / 2.0,
        reference.width * projection_scale,
        reference.height * projection_scale,
    );
    Ok(MasterViewportProjection {
        mode: "centered-fallback",
        viewport_matches,
        scale: projection_scale,
        reference: css_viewport(projected_reference),
    })
}
...
pub(crate) fn native_viewport(
    ...
    let projection = native_viewport_projection(prepared, window)?;
    projection
        .reference
        .to_physical(window.2, window.0, window.1)
}

#[cfg(test)]
mod viewport_projection_tests {
    use super::*;

    fn prepared() -> PreparedMaster {
        PreparedMaster::read(
            include_str!(
    ...
    #[test]
    fn native_viewport_projects_centered_fallback_on_viewport_mismatch() {
        let prepared = prepared();
        let projection = native_viewport_projection(&prepared, (1536, 864, 1.0)).unwrap();
