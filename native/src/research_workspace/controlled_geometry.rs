//! Additive HTML-video proof cache and exact catalogue-v3 live binding.
use super::*;
...
mod tests {
    ...
    fn controlled_catalogue_binds_full_proof_and_rejects_changed_content_or_cache() {
        ...
        receipt.video_width = receipt.display_metadata.video_width_px;
        receipt.video_height = receipt.display_metadata.video_height_px;
        let summary = service
            .attest_native_decode_v2(
                &workspace_id,
                &item.sha256,
            ...
            .is_err());
        {
            ...
            let cached = guard.as_mut().unwrap().scanned[0]
                ...
                .as_mut()
                .unwrap();
            cached.display_metadata.video_width_px = 1;
        }
        assert!(service
            .validate_runner_video_catalogue_v3(&workspace_id, &catalogue)
            .is_err());
...
fn validate_receipt(
        ...
        || receipt.video_height != receipt.display_metadata.video_height_px
    {
        return Err(CommandError::invalid_contract(
            "HTML video controlled decode receipt is incomplete.",
        ));
    }
...
impl WorkspaceService {
        ...
        let candidate = workspace
            ...
            .ok_or_else(|| {
                CommandError::forbidden(
                    "HTML video evidence does not match the latest workspace scan.",
                )
            })?;
        let (hash, bytes) = hash_file(&candidate.path)?;
        if hash != expected_sha256 || bytes != expected_byte_length {
            return Err(CommandError::forbidden(
                "The workspace stimulus changed during HTML video decode preflight.",
            ));
        }
    ...
    pub(crate) fn validate_runner_video_catalogue_v3(
        ...
        for entry in catalogue.entries {
            ...
            let html_geometry = match &entry.geometry {
                VideoDisplayGeometryV3::Controlled(_) => {
                    ...
                    {
                        return Err(CommandError::forbidden(
                            "Controlled HTML video proof cache is inconsistent.",
                        ));
                    }
                ...
                VideoDisplayGeometryV3::Historical(_) => {
                    ...
                    let geometry = candidate.display_geometry.as_ref().ok_or_else(|| {
                        CommandError::forbidden("Historical geometry is missing.")
                    })?;
            ...
            if candidate.decode_status != DecodeStatus::AttestedUnqualified
                ...
                .is_err()
                || entry.geometry != html_geometry
            {
                return Err(CommandError::forbidden(
                    "Current HTML video does not exactly match the saved catalogue proof.",
                ));
            }
            bindings.push(RunnerVideoBindingV3 {
                ...
                display_geometry: html_geometry,
            });
        }
        Ok(bindings)
    }
