...
impl<'de> Deserialize<'de> for VideoDisplayGeometryV3 {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        match value.get("source").and_then(Value::as_str) {
            Some("html-video-controlled-renderer") => {
                serde_json::from_value(value).map(Self::Controlled)
            }
            Some("browser-decoder" | "html-video-metadata") => {
...
impl VideoDisplayGeometryV3 {
    ...
    fn validate(&self) -> ResearchResult<()> {
        match self {
            ...
            Self::Controlled(v) => {
                if derive_native_display_geometry_v2(&v.html_video_metadata)? != *v {
                    return Err(invalid(
                        "Controlled geometry disagrees with its HTML video proof.",
                    ));
                }
                Ok(())
            }
    ...
    fn invalid_nested_proof_or_redundant_geometry_never_passes_with_rehashed_catalogue() {
        for mode in 0..9 {
            ...
            match mode {
                ...
                1 => {
                    geometry["htmlVideoMetadata"]["sourceOrientation"]["stream"] =
