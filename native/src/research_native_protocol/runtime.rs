...
//!
//! One worker owns protocol transitions, HTML video lifecycle observations,
...
impl PackageProtocolRuntime {
    ...
    pub fn start(&self, request: StartPackageRunRequest) -> ResearchResult<PackageStartRunReceipt> {
        ...
        if request.playback_mode != PlaybackMode::UnqualifiedWebview {
            return Err(CommandError::invalid_contract(
                "The Rust-owned package protocol requires native HTML video playback.",
            ));
        }
        ...
        if playback_qualification != PlaybackQualification::QualifiedNative {
            return Err(CommandError::native_media_unavailable(
                "html-video-qualification-required",
            ));
        }
        let loaded =
...
impl Worker {
    ...
    fn start(&mut self) -> ResearchResult<()> {
        if self.resumed {
            ...
            self.write_event(EventContext::session(
                ...
                Some("html-video-package-v1".to_owned()),
            ))?;
            self.write_event(EventContext::session(
    ...
    fn require_media_binding(
        ...
        {
            return Err(CommandError::forbidden(
                "Native HTML video status crossed the frozen run or asset generation.",
            ));
        }
        Ok(())
    }
