...
use crate::research_video_geometry::{
    derive_native_display_geometry_v1, NativeDisplayGeometryV1, NativeDisplayMetadataReceiptV1,
    NativeVideoOrientationV1, VideoRatioV1, NATIVE_DISPLAY_METADATA_SCHEMA,
};
...
pub struct HtmlVideoAssetRequest {
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct HtmlVideoAssetPreparationReceipt {
    ...
    pub prepared: Vec<HtmlVideoPreparedStimulus>,
    pub stimuli: Vec<ScannedStimulusSummary>,
}

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct HtmlVideoPreparedStimulus {
    pub stimulus_id: String,
    pub title: String,
    pub original_relative_path: String,
    pub prepared_relative_path: String,
...
impl WorkspaceService {
    ...
    pub fn rescan_planner_videos(&self, workspace_id: &str) -> ResearchResult<RescanResult> {
        let mut guard = self.lock_selected();
        let workspace = selected_mut(&mut guard, workspace_id)?;
        let package_assets = validate_selected_workspace(workspace)?
            .package_assets
            .clone();
        workspace.scanned = scan_planner_videos(&package_assets)?;
        workspace.media_grants.clear();
        Ok(RescanResult {
            workspace_id: workspace.id.clone(),
    ...
    pub fn prepare_html_video_assets(
        &self,
        workspace_id: &str,
        requests: Vec<HtmlVideoAssetRequest>,
    ) -> ResearchResult<HtmlVideoAssetPreparationReceipt> {
        let (selected_workspace_id, package_assets) = {
            let guard = self.lock_selected();
            let workspace = selected_ref(&guard, workspace_id)?;
            let libraries = validate_selected_workspace(workspace)?;
        ...
        for request in requests {
            let original = resolve_planner_video_asset(&package_assets, &request.relative_path)?;
            if let (Some(expected_sha256), Some(expected_bytes)) =
                (request.sha256.as_deref(), request.byte_length)
            {
                let (observed_sha256, observed_bytes) = hash_file(&original.path)?;
                if observed_sha256 != expected_sha256 || observed_bytes != expected_bytes {
                    return Err(CommandError::forbidden(format!(
                        "Video {} changed before HTML preparation.",
                        request.relative_path
                    )));
                }
            }
            let preparation = prepare_one_html_video_asset(&package_assets, &original)?;
            prepared.push(HtmlVideoPreparedStimulus {
                stimulus_id: request.stimulus_id,
                title: request.title,
                original_relative_path: original.logical_relative_path,
                prepared_relative_path: preparation.prepared_relative_path,
                action: preparation.action,
                reason_code: preparation.reason_code,
            });
        }

        let scanned = scan_planner_videos(&package_assets)?;
        let mut guard = self.lock_selected();
        let workspace = selected_mut(&mut guard, workspace_id)?;
        let libraries = validate_selected_workspace(workspace)?;
        if workspace.id != selected_workspace_id || libraries.package_assets != package_assets {
            return Err(CommandError::forbidden(
                "The selected workspace changed during HTML video preparation.",
            ));
        }
        ...
        for item in &prepared {
            let summary = workspace
                .scanned
                .iter()
                .find(|entry| entry.logical_relative_path == item.prepared_relative_path)
                .map(scanned_summary)
                .ok_or_else(|| {
                    CommandError::forbidden(format!(
                        "Prepared video {} was not found in the current Planner library.",
                        item.prepared_relative_path
                    ))
                })?;
            summaries.push(summary);
        }
        let converted_count = prepared
            .iter()
            .filter(|item| item.action == "converted")
            .count();
        let reused_count = prepared
            .iter()
            .filter(|item| item.action == "reused")
            .count();
        let ready_count = prepared
            .iter()
            .filter(|item| item.action == "alreadyReady")
            .count();
        Ok(HtmlVideoAssetPreparationReceipt {
            schema: "affect-research-html-video-asset-preparation",
            version: 1,
            workspace_id: workspace.id.clone(),
            checked_count: prepared.len(),
            converted_count,
            reused_count,
            ready_count,
            prepared,
    ...
    pub fn validate_planner_video_catalogue(
        ...
        for entry in &catalogue.entries {
            ...
            if candidate.decode_status != DecodeStatus::AttestedUnqualified
    ...
    /// currently selected and HTML-video-attested Planner media closure.
    ...
    pub(crate) fn validate_runner_video_catalogue(
        ...
        for entry in catalogue.entries {
            ...
            let [candidate] = stored_matches.as_slice() else {
                return Err(CommandError::forbidden(
                    "A saved catalogue location does not resolve to one HTML-video-attested video.",
                ));
            };
            let current_matches = current
                .iter()
            ...
            let Some(duration_ms) = duration_ms.map(|duration| duration as u64) else {
                return Err(CommandError::forbidden(
                    "A saved catalogue video has no exact HTML video duration.",
                ));
            };
            let geometry = candidate.display_geometry.as_ref().ok_or_else(|| {
                CommandError::forbidden(
                    "A saved catalogue video has no verified HTML video geometry.",
                )
            })?;
            ...
            {
                return Err(CommandError::forbidden(
                    "A current HTML Runner video no longer matches the saved catalogue.",
                ));
            }
    ...
    pub fn issue_planner_media_url(
        &self,
        workspace_id: &str,
        ...
        let candidate = workspace
            ...
            .iter()
            .find(|entry| {
                entry.logical_relative_path == expected_relative_path
                    && entry.sha256 == expected_sha256
                    && entry.byte_length == expected_byte_length
            })
            .ok_or_else(|| {
                CommandError::forbidden(
                    "The selected JSON video does not match the current Planner video library.",
                )
            })?
            .clone();
        let mut locked_file = open_read_locked(&candidate.path)?;
        ...
        if observed_hash != expected_sha256 || observed_bytes != expected_byte_length {
            return Err(CommandError::forbidden(
                "The workspace stimulus changed during HTML video preparation.",
            ));
        }
        let token = Uuid::new_v4().simple().to_string();
        workspace.media_grants.insert(
        ...
        Ok(MediaUrlReceipt {
            ...
            decode_backend: candidate.decode_backend,
            decode_attestation: candidate.decode_attestation,
            decoded_positions_ms: candidate.decoded_positions_ms,
        })
    }
    ...
    pub(crate) fn attest_native_decode(
        ...
        {
            return Err(CommandError::invalid_contract(
                "Removed native-player decode evidence is incomplete.",
            ));
        }
        ...
        candidate.decode_backend = Some(DecodeBackend::LegacyNativePlayer);
    ...
    /// explicitly unqualified and cannot satisfy a native-player verifier.
    pub fn attest_workspace_decode(
        ...
        let display_geometry =
            derive_native_display_geometry_v1(&NativeDisplayMetadataReceiptV1 {
                schema: NATIVE_DISPLAY_METADATA_SCHEMA,
                version: 1,
                encoded_width_px: video_width,
                encoded_height_px: video_height,
                pixel_aspect_ratio: VideoRatioV1 {
                    numerator: 1,
                    denominator: 1,
                },
                orientation: NativeVideoOrientationV1::Identity,
                snapshot_width_px: video_width,
                snapshot_height_px: video_height,
...
struct ResolvedPlannerVideoAsset {
    path: PathBuf,
    logical_relative_path: String,
}

struct PreparedHtmlVideoAsset {
    prepared_relative_path: String,
    action: &'static str,
...
fn resolve_planner_video_asset(
    package_assets_root: &Path,
    logical_relative_path: &str,
) -> ResearchResult<ResolvedPlannerVideoAsset> {
    let relative = logical_relative_path
        .strip_prefix("stimuli/")
        .ok_or_else(|| {
            CommandError::forbidden(
                "Planner HTML video preparation accepts only stimuli/ workspace paths.",
            )
        })?;
    let canonical_relative = portable_import_relative_path(Path::new(relative))?;
    if canonical_relative != relative {
        return Err(CommandError::forbidden(
            "Planner video paths must already be portable canonical text.",
        ));
    }
    let path = portable_relative_to_path(package_assets_root, &canonical_relative)?;
    let metadata = fs::symlink_metadata(&path).map_err(CommandError::io)?;
    if !metadata.is_file() || metadata.file_type().is_symlink() || !is_video(&path) {
        return Err(CommandError::forbidden(
            "The requested Planner video is not an ordinary supported video file.",
        ));
    }
    let canonical = path.canonicalize().map_err(CommandError::io)?;
    if !canonical.starts_with(package_assets_root) {
        return Err(CommandError::forbidden(
            "A Planner video escaped assets/stimuli/ during HTML preparation.",
        ));
    }
    Ok(ResolvedPlannerVideoAsset {
        path: canonical,
        logical_relative_path: format!("stimuli/{canonical_relative}"),
    })
}

fn prepare_one_html_video_asset(
    package_assets_root: &Path,
    source: &ResolvedPlannerVideoAsset,
) -> ResearchResult<PreparedHtmlVideoAsset> {
    let source_probe = ffprobe_video(&source.path)?;
    if html_video_ready(&source.path, &source_probe) {
        return Ok(PreparedHtmlVideoAsset {
            prepared_relative_path: source.logical_relative_path.clone(),
            action: "alreadyReady",
            reason_code: "already-html-video-ready",
        });
    }
    let stem = source
        .path
        ...
        .ok_or_else(|| CommandError::forbidden("A Planner video name is not valid Unicode."))?;
    if stem.to_ascii_lowercase().ends_with("_converted") {
        return Err(CommandError::forbidden(format!(
            "{} is already a converted sibling but does not satisfy the HTML video contract.",
            source.logical_relative_path
        )));
    }
    let parent = source
        .path
        .parent()
        .ok_or_else(|| CommandError::forbidden("A Planner video has no containing folder."))?;
    let converted = parent.join(format!("{stem}_converted.mp4"));
    let converted_relative_path = logical_relative_path_for_asset(package_assets_root, &converted)?;
    if converted.exists() {
        let metadata = fs::symlink_metadata(&converted).map_err(CommandError::io)?;
        if !metadata.is_file() || metadata.file_type().is_symlink() {
            return Err(CommandError::forbidden(
                "The deterministic converted video target is not an ordinary file.",
            ));
        }
        let target_probe = ffprobe_video(&converted)?;
        if html_video_ready(&converted, &target_probe)
            && converted_is_current_enough(&source.path, &converted)
        {
            return Ok(PreparedHtmlVideoAsset {
                prepared_relative_path: converted_relative_path,
                action: "reused",
                reason_code: "converted-sibling-reused",
            });
        }
    }
    convert_video_for_html(&source.path, &converted)?;
    let converted_probe = ffprobe_video(&converted)?;
    if !html_video_ready(&converted, &converted_probe) {
        return Err(CommandError::forbidden(
            "ffmpeg finished, but the converted video still did not satisfy the HTML video contract.",
        ));
    }
    Ok(PreparedHtmlVideoAsset {
        prepared_relative_path: converted_relative_path,
        action: "converted",
        reason_code: "converted-with-ffmpeg",
    })
}

fn portable_relative_to_path(root: &Path, relative: &str) -> ResearchResult<PathBuf> {
    let mut path = root.to_owned();
    for part in relative.split('/') {
        if part.is_empty() || part == "." || part == ".." {
            return Err(CommandError::forbidden(
                "Planner video paths must use non-empty portable segments.",
            ));
        }
        path.push(part);
    }
...
fn ffprobe_video(path: &Path) -> ResearchResult<FfprobeOutput> {
    let output = Command::new(video_tool("AFFECT_RESEARCH_FFPROBE", "ffprobe"))
        .arg("-v")
        .arg("error")
        .arg("-print_format")
        .arg("json")
        ...
        .map_err(|error| {
            CommandError::forbidden(format!(
                "ffprobe is required to prepare HTML video assets ({error})."
            ))
        })?;
    if !output.status.success() {
        return Err(CommandError::forbidden(format!(
            ...
            path.file_name()
                .and_then(|value| value.to_str())
                .unwrap_or("video"),
            bounded_process_message(&output.stderr)
        )));
    }
    serde_json::from_slice(&output.stdout).map_err(|error| {
        CommandError::invalid_contract(format!("ffprobe returned invalid JSON: {error}."))
    })
}

fn html_video_ready(path: &Path, probe: &FfprobeOutput) -> bool {
    let extension_ready = path
        .extension()
        .and_then(|extension| extension.to_str())
        .is_some_and(|extension| matches!(extension.to_ascii_lowercase().as_str(), "mp4" | "m4v"));
    if !extension_ready {
        return false;
    }
    let container_ready = probe
        .format
        .as_ref()
        .and_then(|format| format.format_name.as_deref())
        .is_some_and(|name| {
            name.split(',')
                .any(|part| matches!(part.trim(), "mp4" | "mov" | "m4v" | "3gp" | "3g2" | "mj2"))
        });
    let video_ready = probe.streams.iter().any(|stream| {
    ...
    let audio_ready = probe
        .streams
        .iter()
        .filter(|stream| stream.codec_type.as_deref() == Some("audio"))
        .all(|stream| matches!(stream.codec_name.as_deref(), Some("aac" | "mp3")));
    container_ready && video_ready && audio_ready
}

fn converted_is_current_enough(source: &Path, converted: &Path) -> bool {
    let source_modified = fs::metadata(source).and_then(|metadata| metadata.modified());
    let converted_modified = fs::metadata(converted).and_then(|metadata| metadata.modified());
    match (source_modified, converted_modified) {
        (Ok(source_time), Ok(converted_time)) => converted_time >= source_time,
        _ => true,
    }
}

fn convert_video_for_html(source: &Path, target: &Path) -> ResearchResult<()> {
    let parent = target
        .parent()
        .ok_or_else(|| CommandError::forbidden("A converted video has no containing folder."))?;
    let staging = parent.join(format!(".{}.html-video.mp4", Uuid::new_v4()));
    let output = Command::new(video_tool("AFFECT_RESEARCH_FFMPEG", "ffmpeg"))
        .arg("-y")
        .arg("-i")
        .arg(source)
        .arg("-map")
        ...
        .map_err(|error| {
            CommandError::forbidden(format!(
                "ffmpeg is required to convert videos for the HTML player ({error})."
            ))
        })?;
    if !output.status.success() {
        let _ = fs::remove_file(&staging);
        return Err(CommandError::forbidden(format!(
            "ffmpeg could not create the HTML-compatible video: {}",
            bounded_process_message(&output.stderr)
        )));
    }
    replace_with_staging(&staging, target)
...
fn video_tool(environment_key: &str, fallback: &str) -> String {
        ...
        .filter(|_| {
            ...
            (entry.decode_status == DecodeStatus::AttestedUnqualified
                ...
                || (entry.decode_status == DecodeStatus::AttestedQualified
                    && entry.decode_backend == Some(DecodeBackend::LegacyNativePlayer)
...
fn native_attested_candidate<'a>(
    ...
    if !expected_duration_ms.is_finite() || expected_duration_ms < 10.0 {
        return Err(CommandError::invalid_contract(
            "A native-player-qualified workspace stimulus requires a complete-video duration.",
        ));
    }
    if expected_relative_path
        != scanned
    ...
    scanned
        ...
        .ok_or_else(|| {
            CommandError::forbidden(
                "The opaque workspace file and its native-player attestation do not match the latest scan.",
            )
        })
}
