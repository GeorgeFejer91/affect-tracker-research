...
fn read_recipe_bytes(path: &Path) -> ResearchResult<Vec<u8>> {
    ...
    if is_link(&metadata) || !metadata.is_file() || metadata.len() == 0 {
        return Err(CommandError::invalid_contract(
...
fn parse_supported_at(path: &Path, bytes: &[u8]) -> ResearchResult<LoadedSupportedPlannerRecipe> {
    ...
    for entry in manifest.references()? {
        ...
        if !metadata.is_file() || metadata.len() != entry.byte_length {
            return Err(CommandError::invalid_contract(
                "Questionnaire file is missing or its size changed.",
            ));
        }
        let source_text = String::from_utf8(read_recipe_bytes(&target)?)
            .map_err(|_| CommandError::invalid_contract("Invalid questionnaire UTF-8."))?;
        snapshots.push(QuestionnaireAssetSnapshot {
...
pub(crate) fn store_questionnaire_snapshots(
    directory: &Path,
    snapshots: &[QuestionnaireAssetSnapshot],
) -> ResearchResult<()> {
    ...
    for snapshot in snapshots {
        ...
        if parts.len() != 5
            || parts[0] != "assets"
            || parts[1] != "questionnaires"
            || parts.iter().any(|part| {
                part.is_empty() || *part == "." || *part == ".." || part.contains(['\\', ':'])
            })
        {
            return Err(CommandError::invalid_contract(
                "Invalid questionnaire snapshot path or size.",
            ));
        }
        ...
        if !destination_exists(&target)? {
            let staged = StagedRecipe(
                parent.join(format!(".affect-questionnaire-{}.staging", Uuid::new_v4())),
            );
            let mut file = OpenOptions::new()
                ...
                .open(&staged.0)
                .map_err(CommandError::io)?;
            file.write_all(snapshot.source_text.as_bytes())
                .map_err(CommandError::io)?;
        ...
        if read_recipe_bytes(&target)? != snapshot.source_text.as_bytes() {
            return Err(CommandError::invalid_contract(
                "Existing questionnaire asset differs; it was not replaced.",
            ));
        }
    }
    Ok(())
...
pub(crate) fn verify_loaded_questionnaire_assets(
    directory: &Path,
    loaded: &LoadedSupportedPlannerRecipe,
) -> ResearchResult<()> {
...
trait FileRecipeDocument: Sized {
    ...
    fn parse_at(_path: &Path, bytes: &[u8]) -> ResearchResult<Self> {
        Self::parse(bytes)
    }
    fn prepare_assets(&self, _directory: &Path) -> ResearchResult<()> {
        Ok(())
    }
...
impl FileRecipeDocument for LoadedSupportedPlannerRecipe {
    ...
    fn parse_at(path: &Path, bytes: &[u8]) -> ResearchResult<Self> {
        parse_supported_at(path, bytes)
    }
...
mod tests {
    ...
    fn manifest5_files_roundtrip_and_changed_assets_never_publish_or_start() {
        ...
        assert_eq!(
            ...
            expected.canonical_source_text
        );
        let loaded = read_supported_planner_recipe_file(&path).unwrap();
        assert_eq!(loaded.transport_text().unwrap(), source);
        assert_eq!(
            read_supported_planner_recipe_path(&path).unwrap()["kind"],
            "planner-recipe-v5"
        );
        verify_loaded_questionnaire_assets(&root.0, &loaded).unwrap();
        let assets = match &loaded.recipe {
            SupportedPlannerRecipe::V5(v) => &v.assets,
            _ => panic!(),
        };
        for asset in assets {
            assert_eq!(
                fs::read_to_string(root.0.join(&asset.relative_path)).unwrap(),
                asset.source_text
            );
        }
        ...
        assert!(
            write_selected_supported_planner_recipe(&root.0.join("rejected.json"), source).is_err()
        );
