...
const PLAYBACK_MODES = new Set(["unqualifiedWebview"]);
...
function expectedPlaybackQualification(playbackMode) {
  if (playbackMode !== "unqualifiedWebview") throw new TypeError("Unknown native playback mode.");
  return "unqualified";
}
...
export function validateNativeMediaCapabilityV2(value) {
  ...
  if (keys.length !== NATIVE_MEDIA_CAPABILITY_KEYS.length
    ...
    || value.version !== 2
    || value.backend !== "html-video-element"
    || value.api !== "research-media"
    || value.pinnedRuntimeVersion !== "none"
    || value.bindingsVersion !== "webview"
    || value.target !== "tauri-webview"
    ...
    throw new TypeError("Native media capability v2 is malformed or does not match the HTML video protocol contract.");
  }
  if (value.runtimeIntegrityVerified
    || value.playerActorReady
    || value.qualifiedStartAvailable
...
export function nativeRunStatusHandshake(status) {
  ...
  if (status.active) {
    return RUN_ID_PATTERN.test(status.runId ?? "")
      ...
      && status.playbackMode === "unqualifiedWebview"
      && status.playbackQualification === "unqualified";
  }
...
export function authorizeDesktopPlaybackMode(requestedMode, capability) {
  ...
  throw new Error(`Native child-window playback is retired (${validated.reasonCode}). Use WebView HTML video.`);
}
...
export class NativeResearchRuntimeBridge {
  ...
  destroy() {
    ...
    this.run = null;
    if (!this.plannerOnly) this.#clearVideo();
  }
    ...
    this.#listen(this.root, RESEARCH_UI_EVENTS.stimulusAuthoringRequest, (event) => {
      ...
      this.#queue(async () => {
        try {
          ...
          } else if (operation === "prepare-html-video-assets") {
            const result = await this.invoke("research_prepare_html_video_assets", {
              workspaceId,
              stimuli: Array.isArray(payload.stimuli) ? payload.stimuli : [],
            });
            const preparedSettings = {
              stimuli: {
                items: result.prepared.map((item) => ({
                  stimulusId: item.stimulusId,
                  title: item.title,
                  source: { kind: "workspaceFile", relativePath: item.preparedRelativePath },
                })),
              },
            };
            const current = () => workspaceId === this.workspace?.workspaceId;
            const publication = await this.prepareCatalogue(result, {
              settings: preparedSettings,
              isCurrent: current,
              reportProgress: true,
            });
            publication.commit();
            this.#dispatch(RESEARCH_UI_EVENTS.stimuliCatalogued, publication.projection);
            receipt = { ...result, projection: publication.projection };
  ...
  async #refreshNativeMediaViewport() {
    // HTML video is laid out inside the WebView; there is no native child
    // renderer viewport to synchronize.
  }
  ...
  async ensureMediaReady({ isCurrent = () => true, signal, deadline } = {}) {
    ...
    if (this.#selectedPlaybackMode() !== "unqualifiedWebview") {
      throw new Error("Only the lightweight WebView HTML video protocol is supported.");
    }
    const baseCurrent = this.#catalogueCurrent();
    const current = () => isCurrent() && baseCurrent();
    if (signal?.aborted || !current()) throw new Error("HTML video readiness was superseded.");
    const capability = validateNativeMediaCapabilityV2(await this.invoke("research_native_media_capability"));
    if (signal?.aborted || !current()) throw new Error("HTML video readiness was superseded.");
    ...
    for (const scanned of scannedStimuli) {
      ...
      try {
        ...
        const validFallback = summary.decodeStatus === "attestedUnqualified"
          && summary.decodeBackend === "webviewVideoFrameCallback"
          && summary.decodeAttestation === "representativeFramesV1";
        if (!validFallback || !summary.source || !summary.displayGeometry) {
          throw new Error(`${summary.displayName} did not produce the WebView HTML video decode and geometry contract.`);
        }
        ...
        items.push(Object.freeze({
          ...
          decodeQualification,
          workspaceFileId: summary.workspaceFileId,
          displayGeometry: summary.displayGeometry ? structuredClone(summary.displayGeometry) : null,
        }));
      } catch (error) {
        check();
  ...
  async #startPackage(detail) {
    ...
    if (detail?.playbackMode !== "unqualifiedWebview") {
      throw new Error("Reproducible experiment packages require Rust-owned native HTML video playback.");
    }
    const current = this.#catalogueCurrent();
    const result = await this.invoke("research_rescan_package_stimuli", {
      workspaceId: this.workspace.workspaceId,
  ...
  async #prepareCurrentStimulus(fence, { recovery = false } = {}) {
    ...
    if (entry.summary.decodeStatus !== "attestedUnqualified"
      || entry.summary.decodeBackend !== "webviewVideoFrameCallback"
      || entry.summary.decodeAttestation !== "representativeFramesV1") {
      throw new Error(`${stimulus.title} lacks a fresh WebView HTML video frame attestation.`);
    }
    ...
    this.#dispatch(RESEARCH_UI_EVENTS.runStatus, {
      ...
      transitionMessage: recovery
        ...
        : "The complete video passed the WebView frame attestation. Begin when ready.",
  ...
  #mediaTimeMs(run = this.run) {
    const _ = run;
    return Math.max(0, Number(this.#video().currentTime) * 1_000 || 0);
  }
