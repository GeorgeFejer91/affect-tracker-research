...
function questionnairesSection() {
  return `
    ...
    <dialog id="questionnaire-survey-import" class="research-dialog questionnaire-sheet-preview" aria-labelledby="questionnaire-survey-title">
      <div class="dialog-heading"><h2 id="questionnaire-survey-title">Import SurveyJS questionnaire</h2><button type="button" data-survey-import-close>Cancel</button></div>
      <p>Copy the questionnaire from the SurveyJS builder’s JSON Editor tab, or paste JSON generated by your Node.js code.</p>
      <textarea id="questionnaire-survey-json" rows="14" aria-label="SurveyJS questionnaire JSON" spellcheck="false"></textarea>
      <p id="questionnaire-survey-error" role="status"></p><button type="button" id="questionnaire-survey-apply">Import questionnaire</button>
    </dialog>
        ...
        <section class="run-mode" data-mode-panel="run" aria-label="Running the Experiment" hidden>
          ...
          <div class="run-stage">
            <section class="stimulus-stage" aria-label="Current complete stimulus">
              <div id="run-native-video-host" class="native-video-host" aria-label="Protocol-controlled HTML video stimulus surface" hidden></div>
