...
function bindResearchInteractions(root, { surface }) {
  ...
  let previewFlubberReleased = false;
  let previewReleasedPosition = { x: 0.5, y: 0.5 };
  ...
  function previewState({ locked = false, design = false } = {}) {
    ...
    const releasedFlubber = design && feedbackSettingsVersion === 2 && previewFlubberReleased;
    ...
    return {
      ...
      position: design && feedbackSettingsVersion === 2 ? (releasedFlubber ? previewReleasedPosition : { x: 0.5, y: 0.5 })
        : { x: numberValue("visual-position-x", 0.5), y: numberValue("visual-position-y", 0.5) },
      lockPosition: locked || (design && feedbackSettingsVersion === 2 ? !releasedFlubber : checked("visual-lock-position")),
      ...(design ? {
        displayMode: feedbackSettingsVersion === 2 ? (releasedFlubber ? "flubber" : feedbackPreviewMode) : "legacy",
  ...
  function renderPreviewDesignControls() {
    ...
    if (releaseButton instanceof HTMLButtonElement) {
      ...
      releaseButton.setAttribute("aria-pressed", String(!legacy && previewFlubberReleased));
    }
  ...
  function applyFeedbackFields(normalized) {
    ...
    if (feedbackSettingsVersion === 2) {
      ...
      previewFlubberReleased = false;
      previewReleasedPosition = { x: 0.5, y: 0.5 };
      ...
      },
      {
        ...
        message: stimuliReady && poolCapacity.valid
          ? hasQualifiedNativeDecode
            ? `${stimuli.length} complete video${stimuli.length === 1 ? "" : "s"} covered, byte-bound, and prepared for HTML video playback`
      ...
      verification.textContent = stimulus.source === "youtube"
        ...
        : stimulus.verification === "verified"
          ? stimulus.decodeQualification === "attestedQualified"
            ? "Hash + HTML video playback metadata attested"
  ...
  async function acceptPlannerSegmentForCurrentSave(segment, { isCurrent }) {
    const current = () => !researchUiDisposed && mode === "setup" && isCurrent();
    let preparedOwner = null;
    if (segment === "P3") preparedOwner = await stimulusOrderEditor.prepareConfirmation({ isCurrent: current });
    if (segment === "P4") preparedOwner = await layoutDraftEditor.prepareConfirmation({ isCurrent: current });
    if (segment === "P6") preparedOwner = await xrLayoutAuthoring.prepareConfirmation({ isCurrent: current });
    const prepared = await plannerContributions.prepareAcceptance(segment, {
      selectedTarget: getSelectedPlannerTarget(),
      isCurrent: current,
      preparedOwner,
    });
    if (!current() || !prepared.isCurrent()) throw new Error("HTML video preparation changed before planner confirmation.");
    prepared.commit();
    prepared.afterCommit();
  }

  async function prepareHtmlVideoAssetsForPlannerSave() {
    if (surface !== "tauri" || root.dataset.nativeWorkspaceReady !== "true") return null;
    const currentWorkspaceKey = authoringWorkspaceKey;
    const isCurrent = () => !researchUiDisposed && mode === "setup"
      && authoringWorkspaceKey === currentWorkspaceKey;
    const videoRequests = stimuli
      .filter((stimulus) => stimulus.source === "workspace"
        && stimulus.verification === "verified"
        && stimulus.contractSource?.kind === "workspaceFile")
      .map((stimulus) => ({
        stimulusId: stimulus.id,
        title: stimulus.title,
        relativePath: stimulus.contractSource.relativePath,
        sha256: stimulus.contractSource.sha256,
        byteLength: stimulus.contractSource.byteLength,
      }));
    if (videoRequests.length === 0) return null;
    const receipt = await operateStimulusAuthoring("prepare-html-video-assets", {
      stimuli: videoRequests,
    });
    if (!isCurrent()) throw new Error("The workspace changed during HTML video preparation.");
    await refreshVideoCatalogueContribution();
    if (!isCurrent()) throw new Error("The workspace changed after HTML video preparation.");
    const sourceChanged = receipt.prepared.some((item) => (
      item.originalRelativePath !== item.preparedRelativePath
    ));
    if (sourceChanged) {
      await acceptPlannerSegmentForCurrentSave("P1", { isCurrent });
      await acceptPlannerSegmentForCurrentSave("P3", { isCurrent });
      await acceptPlannerSegmentForCurrentSave("P4", { isCurrent });
      if (getSelectedPlannerTarget() !== null) {
        await acceptPlannerSegmentForCurrentSave("P6", { isCurrent });
      }
    }
    return receipt;
  }
  ...
  async function generateExperimentPackage({ reexport = false } = {}) {
    if (!languageEditorLocked || experimentPackageDocument?.recipe) {
      try {
        const preparation = await prepareHtmlVideoAssetsForPlannerSave();
        if (preparation?.checkedCount > 0) {
          const changed = preparation.convertedCount + preparation.reusedCount;
          announce(changed
            ? `${preparation.checkedCount} video file${preparation.checkedCount === 1 ? "" : "s"} checked for the HTML player; ${preparation.convertedCount} converted and ${preparation.reusedCount} reused.`
  ...
  root.addEventListener("click", (event) => {
    ...
    if (target.id === "preview-flubber-release") {
      if (feedbackSettingsVersion !== 2) return;
      const changedRenderer = feedbackPreviewMode !== "flubber";
      feedbackPreviewMode = "flubber";
      previewFlubberReleased = true;
      previewReleasedPosition = { x: 0.5, y: 0.5 };
      if (changedRenderer) {
        packageEditRevision += 1;
        markPlannerEdit();
      ...
      refreshProjection();
      query(".preview-primary-stage")?.focus();
      announce("Flubber released in the preview. Drag it inside the preview field.");
      return;
    }
