...
function htmlVideoViewportCssV1(host, layoutRevision) {
  const rect = host?.getBoundingClientRect?.();
  if (!rect || !Number.isSafeInteger(layoutRevision) || layoutRevision < 1) {
    throw new TypeError("HTML video viewport requires a rendered host and positive layout revision.");
  }
  const values = [rect.left, rect.top, rect.width, rect.height];
  if (values.some((value) => typeof value !== "number" || !Number.isFinite(value))
    || rect.left < 0 || rect.top < 0 || rect.width < 1 || rect.height < 1) {
    throw new TypeError("HTML video host geometry is unavailable.");
  }
...
export function validateNativePackageProtocolCapabilityV1(value) {
  if (!exactKeys(value, CAPABILITY_KEYS)
    ...
    || value.version !== 1
    || value.backend !== "html-video-package-protocol"
...
export function validateNativePackageStartReceiptV1(value) {
  if (!exactKeys(value, START_RECEIPT_KEYS)
    ...
    || value.playbackMode !== "unqualifiedWebview"
...
export class NativePackageProtocolAdapter {
  ...
  #viewport() {
    ...
    return htmlVideoViewportCssV1(host, revision);
  }
