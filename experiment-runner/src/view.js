...
export function runnerMarkup() {
  return `<div class="runner-shell">
    <main id="runner-launcher" class="runner-launcher">
      ...
      <div class="runner-load-actions"><button id="runner-open">Load new experiment</button><select id="runner-recent-files" aria-label="Previously used experiment JSON files" disabled><option value="">No previous files</option></select></div>
      ...
      <div id="runner-variant-field" class="runner-version-picker" hidden>
        ...
        <div class="runner-version-combobox"><button id="runner-variant-button" type="button" role="combobox" aria-haspopup="listbox" aria-expanded="false" aria-controls="runner-variant-options" aria-labelledby="runner-variant-heading runner-variant-label" aria-describedby="runner-variant-status"><span><strong id="runner-variant-label">Choose version</strong><span id="runner-variant-count"></span></span><span class="runner-version-chevron" aria-hidden="true">▾</span></button>
        <div id="runner-variant-popup" hidden><p id="runner-variant-scale"></p><div class="runner-version-legend" aria-hidden="true" hidden></div><div id="runner-variant-options" role="listbox" aria-label="Experiment versions"></div></div></div>
    ...
    <dialog id="runner-controller-dialog" class="runner-dialog" aria-labelledby="runner-controller-title"><header class="runner-dialog-header"><h2 id="runner-controller-title">Set controller</h2><button data-close-dialog="runner-controller-dialog">Done</button></header><p id="runner-controller-status" class="runner-controller-source">Load an experiment to see its configured input.</p><div id="runner-controller-overview" class="runner-controller-overview" aria-live="polite" data-state="empty"><span id="runner-controller-overview-state">Waiting</span><strong id="runner-controller-overview-target">Experiment file</strong><output id="runner-controller-overview-input">Load file</output></div><div class="runner-controller-pad" role="group" aria-label="Controller movement bindings" aria-describedby="runner-controller-overview runner-controller-note">
      <button type="button" class="runner-controller-cell runner-controller-up" data-controller-capture="up" aria-label="Set up movement"><span aria-hidden="true">↑</span><strong>Up</strong><output data-controller-binding-value="up">Unassigned</output></button>
      <button type="button" class="runner-controller-cell runner-controller-left" data-controller-capture="left" aria-label="Set left movement"><span aria-hidden="true">←</span><strong>Left</strong><output data-controller-binding-value="left">Unassigned</output></button>
      <button type="button" class="runner-controller-cell runner-controller-neutral" data-controller-capture="neutral" aria-label="Set neutral reset hotkey"><span aria-hidden="true">•</span><strong>Neutral</strong><output data-controller-binding-value="neutral">Set hotkey</output></button>
      <button type="button" class="runner-controller-cell runner-controller-right" data-controller-capture="right" aria-label="Set right movement"><span aria-hidden="true">→</span><strong>Right</strong><output data-controller-binding-value="right">Unassigned</output></button>
      <button type="button" class="runner-controller-cell runner-controller-down" data-controller-capture="down" aria-label="Set down movement"><span aria-hidden="true">↓</span><strong>Down</strong><output data-controller-binding-value="down">Unassigned</output></button>
    </div><div class="button-row"><button id="runner-controller-reset">Use settings from file</button></div><p id="runner-controller-note" role="status"></p></dialog>
