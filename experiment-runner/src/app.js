...
export async function bootRunner(root, { invoke, windowObject = window, pollMs = 250, subscribeAbort = () => () => {} } = {}) {
  ...
  const masterProtocol = new NativeMasterProtocolAdapter({ invoke, windowObject, render: renderMaster, fail, terminal: async status => {
    if (destroyed) return;
    preflight = null; inputReceipt = null; clearQuestionnaire(); masterVideoPlayback = null; validationVideo.stop();
  ...
  async function renderMaster(status, plan) {
    ...
    if (step.kind === "questionnaire") {
      ...
      if (videoStep && status.phase === "awaitingPresentation") {
      ...
      } else if (videoStep && status.phase === "preparing") {
        const key = `${status.runId}:${status.position}`;
        ...
        if (masterVideoPlayback) masterVideoPlayback.state = "paused";
      } else if (videoStep && status.phase === "playing") {
        if (masterVideoPlayback?.state === "paused") await validationVideo.resume();
  ...
  async function startAttempt() {
    ...
    if (disposition !== "finalize") {
      if (recipe.recipe) {
        if (!preflight.htmlVideoStartReady) throw new Error("HTML video experiment playback is not ready for this selection.");
      } else {
        throw new Error("Legacy native experiment playback is retired in this build.");
      }
