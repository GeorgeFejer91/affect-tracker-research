...
function versionLabel(index, variant) {
  const ordinal = `V${index + 1}`;
  const title = String(variant?.title ?? "").trim();
  return title && title.toLowerCase() !== ordinal.toLowerCase() ? `${ordinal}: ${title}` : ordinal;
}
function usageLabel(count) {
  return count ? `${count.recordingCount} XDF, ${count.participantCount} participant${count.participantCount===1?"":"s"}` : "Usage unavailable";
}
...
export function createVariantPicker(root, {onChange}) {
  ...
  function render(){
    ...
    q("runner-variant-label").textContent=index<0?"Choose version":versionLabel(index,variants[index]);
    q("runner-variant-count").textContent=usageLabel(current);
    ...
    status.textContent=usage?`${manual?"Manual selection":"Auto-selects least-used version"}. Stopped runs count.${usage.ignoredXdfFiles?` ${usage.ignoredXdfFiles} older file${usage.ignoredXdfFiles===1?"":"s"} ignored.`:""}`:"Recording inventory unavailable. Choose a version manually.";
    ...
    for(let i=0;i<variants.length;i++){
      ...
      label.textContent=versionLabel(i,variant);number.textContent=usageLabel(count);option.setAttribute("aria-label",`${label.textContent}, ${number.textContent}`);
    ...
    q("runner-variant-scale").textContent=usage
      ? min===max ? `All versions have ${min} XDF.`
        : `Usage range: ${min} to ${max} XDF.`
      : "Recording counts unavailable";
    root.querySelector(".runner-version-legend").hidden=true;
  }
