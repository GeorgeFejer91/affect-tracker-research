...
import { compilePlannerRecipeV2 } from "../experiment-planner/web/src/research/planner-recipe.js";
...
// Exact outputs of Main's plannerRecipeV2Fixture at owner commit 30291fc.
// Synthetic engineering assets; no actual experiment or media qualification.
let fixturePromise;
const fixture = () => fixturePromise ??= Promise.all(["runner-master-v2-owner", "runner-master-v2-owner-selections"].map(async name => JSON.parse(await readFile(new URL(`./fixtures/${name}.canonical.json`, import.meta.url), "utf8")))).then(([recipe, selections]) => ({ recipe, selections }));
const bytes = value => new TextEncoder().encode(`${canonicalJson(value)}\n`);

test("Runner explicitly reads master2 and preserves all owner selections and typed content", async () => {
  ...
  assert.equal(receipt.recipe.version, 2);
  for (const selected of selections) {
    ...
    assert.equal(plan.version, 2); assert.equal(plan.algorithmVersion, "master-sequence-v2");
  ...
  for (const version of [0, 3, "2"]) await assert.rejects(readRunnerRecipe(bytes({ ...recipe, version })));
...
test("v2 adapter uses participant-only Start, shared status and tagged actions without text coercion", async () => {
  const { adapter, calls, plan } = adapterFixture(2);
  await assert.rejects(adapter.start(plan, { version: 2, participantId: "P001", participant: {} }), /legacy/u);
  assert.equal(calls.length, 0);
  await adapter.start(plan, { version: 2, participantId: "P001" });
  assert.deepEqual(calls.slice(0, 2).map(call => call.name), ["research_runner_master_start_v2", "research_runner_master_status"]);
  ...
  const call = calls.at(-1); assert.equal(call.name, "research_runner_master_action_v2");
  assert.deepEqual(call.args.action, { type: "draft", position: 1, answers: Object.entries(answers).map(([itemId, value]) => ({ itemId, value })) });
  answers.fullName.text = "changed"; assert.equal(call.args.action.answers[0].value.text, "  Jörg\nExample  ");
...
async function informationV2(languageId) {
  const source = await fixture(), { integrity, ...core } = structuredClone(source.recipe);
  core.policy.lsl.enabled = true;
  const recipe = await compilePlannerRecipeV2(core), receipt = await readRunnerRecipe(bytes(recipe));
...
test("startup2 reconstructs EN/DE typed and unchanged Likert rows without legacy identity preparation", async () => {
  for (const language of ["en", "de"]) {
    const f = await informationV2(language), result = await inspectInformationStream(await frameRecords(f.records, f.context));
    ...
    assert.equal((await inspectMasterStream(markers, { planVersion: 2 })).status, "complete");
  }
});

test("information2 rejects empty submissions, type/version confusion and extra legacy identity", async () => {
  const f = await informationV2("en"), index = f.records.findIndex(record => record.kind === "responses" && record.value.responses[0].value);
  for (const mutate of [
