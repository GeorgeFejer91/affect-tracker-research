...
import { requestPlannerFile, PLANNER_LOAD_REQUEST, PLANNER_SAVE_REQUEST } from "../experiment-planner/web/src/research/planner-file-request.js";
...
test("Planner startup adopts an existing workspace even when media rescan fails", async () => {
  const root = new EventTarget(), win = new EventTarget(), calls = [];
  ...
  assert.match(status.textContent, /rescan must wait/u);
...
test("desktop playback defaults to the HTML-compatible WebView mode", () => {
...
test("native media capability v2 is exact, HTML-video-only, and internally consistent", () => {
...
function htmlVideoDisplayGeometry(videoWidth = 1_920, videoHeight = 1_080) {
  const gcd = (left, right) => (right === 0 ? left : gcd(right, left % right));
  const divisor = gcd(videoWidth, videoHeight);
  return {
    status: "verified",
    source: "html-video-metadata",
    displayWidthPx: videoWidth,
    displayHeightPx: videoHeight,
    displayAspect: { numerator: videoWidth / divisor, denominator: videoHeight / divisor },
    rotationDegrees: 0,
...
test("desktop entrypoint sequences the shared UI before the path-free Research native bridge", async () => {
  const [html, entrySource, source, appSource] = await Promise.all([
    readFile(new URL("../experiment-planner/desktop/index.html", import.meta.url), "utf8"),
    readFile(new URL("../experiment-planner/web/src/research/native-entry.js", import.meta.url), "utf8"),
    readFile(new URL("../experiment-planner/web/src/research/native-bridge.js", import.meta.url), "utf8"),
  ...
  for (const command of [
    ...
    "research_prepare_html_video_assets",
  ...
  assert.match(source, /prepare-html-video-assets/u);
  assert.match(source, /probeAndAttestNativeVideo/u);
...
async function preparedBridgeFixture() {
  ...
  const bridge = new NativeResearchRuntimeBridge(root, { windowObject: win,
    setIntervalObject: () => 1, clearIntervalObject: () => {}, invoke: async (command, payload) => {
      ...
      if (command === "research_workspace_media_url") return {
      ...
      if (command === "research_attest_workspace_decode") {
        ...
        actorCalls.push([attestation.attestationKind, attestation.workspaceFileId]);
        if (attestation.attestationKind === "revokeGrant") return {};
        return {
          workspaceFileId: attestation.workspaceFileId,
          displayName: `${attestation.workspaceFileId}.mp4`,
          sha256: attestation.sha256,
          byteLength: attestation.byteLength,
          mimeType: attestation.mimeType,
          durationMs: attestation.observedDurationMs,
          decodeStatus: "attestedUnqualified",
          decodeBackend: "webviewVideoFrameCallback",
          decodeAttestation: "representativeFramesV1",
          decodedPositionsMs: attestation.decodedPositionsMs,
          displayGeometry: htmlVideoDisplayGeometry(attestation.videoWidth, attestation.videoHeight),
          source: { kind: "workspaceFile", relativePath: `stimuli/${attestation.workspaceFileId}.mp4`,
            mimeType: attestation.mimeType, sha256: attestation.sha256, byteLength: attestation.byteLength,
            durationMs: attestation.observedDurationMs },
        };
      }
      return {};
...
test("prepared catalogue uses existing sequential authority without early state/events/progress", async () => {
  ...
  assert.deepEqual(f.actorCalls.map(([name]) => name), ["attestRepresentativeFramesV1"]);
...
test("prepared catalogue guards caller, workspace, mode, settings, capability, newer catalogue and destruction", async () => {
  for (const change of ["caller", "workspace", "mode", "settings", "capability", "catalogue", "destroy"]) {
    ...
    if (change === "mode") f.mode.value = "browserMediaAdapters";
    if (change === "settings") f.root.researchUi.settings.stimuli.items.push({ stimulusId: "new", source: { relativePath: "new.mp4" } });
    if (change === "capability") f.bridge.nativeMediaCapability = { ...f.bridge.nativeMediaCapability, reasonCode: "changed-capability" };
...
test("a canceled pending HTML video probe cannot publish or change progress", async () => {
  const f = await preparedBridgeFixture(); f.connector.prepareWorkspace(preparedWorkspaceReceipt()).commit();
  ...
  assert.equal(f.actorCalls.length, 1);
  f.bridge.destroy();
});
...
test("failed or duplicate catalogue preparation never partially accepts verified entries", async () => {
  for (const failure of ["decode", "duplicate"]) {
    ...
    if (failure === "decode") f.bridge.invoke = async (command, payload) => {
      if (command === "research_attest_workspace_decode"
        && payload.attestation.workspaceFileId === "two"
        && payload.attestation.attestationKind === "attestRepresentativeFramesV1") throw Error("Synthetic decode failure");
      return invoke(command, payload);
    };
...
test("native scan receipts are fenced before awaiting I/O, including same-workspace newer publication", async () => {
  for (const changeWorkspace of [false, true]) {
    ...
    assert.equal(f.actorCalls.filter(([kind]) => kind === "attestRepresentativeFramesV1").length, 1);
    f.bridge.destroy();
  }
});
