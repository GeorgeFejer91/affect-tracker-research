...
export function runnerMarkup() {
  return `<div class="runner-shell">
    ...
    <main id="runner-participant-view" class="runner-presentation" hidden>
      ...
      <section id="runner-questionnaire" class="runner-participant-page" hidden><h2 id="runner-questionnaire-title"></h2><p id="runner-questionnaire-instructions"></p><p id="runner-questionnaire-progress"></p><form id="runner-questionnaire-form"><div id="runner-questionnaire-items"></div><div class="button-row"><button type="button" id="runner-questionnaire-previous">Previous</button><button type="button" id="runner-questionnaire-next">Next</button><button type="submit" id="runner-questionnaire-submit" class="primary-action">Submit responses</button></div></form></section>
    ...
    <dialog id="runner-professor-dialog" class="runner-dialog" aria-labelledby="runner-professor-title"><header class="runner-dialog-header"><h2 id="runner-professor-title">Absent-minded professor</h2><button data-close-dialog="runner-professor-dialog">Done</button></header><p>Connect an experimenter browser to monitor and conduct this Runner over the Internet.</p><label class="field"><input id="professor-video" type="checkbox" checked> Share a low-resolution preview of the current video</label><label class="field"><input id="professor-wait" type="checkbox"> Let Professor start after local participant preparation</label><div class="button-row"><button id="professor-enable">Enable companion</button><button id="professor-disable" disabled>Disable</button><button id="professor-copy" disabled>Copy pairing link</button></div><img id="professor-qr" class="runner-qr" width="320" height="320" alt="Private invitation to this Runner" hidden><p id="professor-status" role="status">Disabled. Signaling uses VDO.Ninja; no camera or microphone is shared.</p><p id="professor-armed-status" role="status"></p><button id="professor-cancel-arm">Cancel remote Start preparation</button></dialog>
    ...
    <dialog id="runner-session-dialog" class="runner-dialog" aria-labelledby="runner-session-title"><header class="runner-dialog-header"><h2 id="runner-session-title">Session controls</h2><button data-close-dialog="runner-session-dialog">Return to experiment</button></header><p id="runner-session">No active session</p><div class="button-row"><button id="runner-pause" disabled>Pause</button><button id="runner-stop" disabled>Stop attempt</button><button id="runner-session-professor">Professor connection</button></div><div class="runner-run-status"><p id="runner-stimulus">Waiting for a recipe</p><p id="runner-timing">Sampling stopped</p><p id="runner-write">No attempt records created</p><p id="runner-lsl">LSL output stopped</p></div></dialog>
