...
export class NativeMasterProtocolAdapter {
  ...
  async start(plan, request, {validation = false} = {}) {
    ...
    if (![1, 2, 3, 4, 5].includes(plan.version)) throw new Error("Unsupported master plan version.");
    if ([2, 3, 4, 5].includes(plan.version) && (request.version !== plan.version || request.participantId !== plan.participantId || Object.hasOwn(request, "participant"))) throw new Error("Typed master Start requires its participant ID without legacy participant preparation.");
    if (validation && ![3, 4, 5].includes(plan.version)) throw new Error("Validation sessions require master3, master4 or master5.");
    const receipt = validation
      ? await this.invoke(plan.version === 5 ? "research_runner_master_validation_start_v5" : "research_runner_master_validation_start", {request:{version:1,acknowledgeUnqualified:true,experiment:request}})
      : await this.invoke(({1:"research_runner_master_start",2:"research_runner_master_start_v2",3:"research_runner_master_start_v3",4:"research_runner_master_start_v4",5:"research_runner_master_start_v5"}[plan.version]), { request });
  ...
  answerAction(type, detail) {
    if (detail.surveyjs) {
      if (![4, 5].includes(this.plan.version)) throw new Error("SurveyJS answers require master version 4.");
