...
export async function attestMasterMedia({ recipe, stimuli, ...options }) {
  if (recipe.version === 1 || recipe.version === 2) return attestNativeGstCatalogue({ stimuli, ...options });
  if (recipe.version !== 3 || recipe.segments.P1.videoCatalogue.version !== 3) throw new Error("Expected a supported master media catalogue.");
